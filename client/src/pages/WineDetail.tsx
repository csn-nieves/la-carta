...
import type { WineDetail as WineDetailType, WineRating, WineReviewReply } from '../types';
...
export default function WineDetail() {
  ...
  // Delete
  const [deleteWineConfirm, setDeleteWineConfirm] = useState(false);
  ...
  const fetchWine = async () => {
    try {
      const { data } = await api.get<{ wine: WineDetailType }>(`/wines/${id}`);
      setWine(data.wine);
    } catch (err) {
  ...
  const handleSubmitRating = async (e: React.FormEvent) => {
    ...
    try {
      await api.post(`/wines/${id}/ratings`, {
        stars,
  ...
  const handleSubmitReply = async (ratingId: string) => {
    ...
    try {
      await api.post(`/wines/${id}/ratings/${ratingId}/replies`, {
  ...
  const handleDeleteWine = async () => {
    try {
      ...
      navigate('/tasks/wines');
    } catch (err) {
      console.error('Failed to delete wine', err);
    } finally {
      setDeleteWineConfirm(false);
    }
  };
  ...
  const handleDeleteRating = async () => {
    ...
    try {
      await api.delete(`/wines/${id}/ratings/${deleteRatingId}`);
      fetchWine();
    } catch (err) {
  ...
  const handleDeleteReply = async () => {
    ...
    try {
      await api.delete(`/wines/${id}/ratings/${deleteReplyInfo.ratingId}/replies/${deleteReplyInfo.replyId}`);
      fetchWine();
    } catch (err) {
  ...
  const canDeleteWine = wine && (wine.createdBy.id === user?.id || isAdmin);
  const hasRated = wine?.userRating !== null;

  return (
    <div className="pb-16 md:pb-6">
      <div className="flex items-center gap-3 mb-6">
        <BackButton to="/tasks/wines" />
        <h1 className="text-3xl font-bold text-neutral-900 dark:text-neutral-100">Wine</h1>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="p-5 rounded-lg bg-neutral-100 dark:bg-neutral-800 mb-6">
            <h2 className="text-2xl font-bold text-neutral-900 dark:text-neutral-100">{wine.name}</h2>
            <div className="flex items-center gap-2 mt-2 text-neutral-600 dark:text-neutral-400">
              ...
              <span>{wine.locationPurchased}</span>
            </div>
            {wine.averageRating !== null && (
              <div className="flex items-center gap-2 mt-3">
                <StarDisplay rating={wine.averageRating} size={18} />
                <span className="text-sm text-neutral-500">
                  {wine.averageRating.toFixed(1)} ({wine.ratingCount} {wine.ratingCount === 1 ? 'rating' : 'ratings'})
                </span>
              </div>
            )}
            <div className="flex items-center justify-between mt-3">
              <span className="text-sm text-neutral-400">
                Added by {wine.createdBy.name}
              </span>
              {canDeleteWine && (
                <button
                  onClick={() => setDeleteWineConfirm(true)}
          ...
          {!hasRated && (
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-neutral-900 dark:text-neutral-100 mb-3">Rate this wine</h3>
                    ...
                    <div key={rating.id} className="p-4 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900">
                      ...
                      {rating.replies.length > 0 && (
                        <div className="mt-3 space-y-3">
                          {rating.replies.map((reply: WineReviewReply) => {
        ...
        <div className="text-center py-20 text-neutral-400">Wine not found</div>
      )}
      ...
      {deleteWineConfirm && (
        <ConfirmModal
          ...
          onCancel={() => setDeleteWineConfirm(false)}
        />
      )}
      {deleteRatingId && (
        <ConfirmModal
