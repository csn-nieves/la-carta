...
export default function CocktailDetail() {
  ...
  return (
    <>
            ...
            {cocktail.tags && cocktail.tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-2">
                {cocktail.tags.map((tag) => (
                  <span
                    key={tag.id}
                    className="px-2.5 py-0.5 rounded-full text-xs bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-300"
                  >
                    {tag.name}
                  </span>
                ))}
      ...
      {showDeleteModal && (
        <ConfirmModal
          message={`Are you sure you wish to delete ${cocktail.name}?`}
